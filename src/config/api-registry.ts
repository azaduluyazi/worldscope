...
export function getApiRegistry(): ApiRegistryEntry[] {
  ...
  return [
    ...
    // ═══ NEW: RepoScout-discovered sources ═══
    { name: "ENTSO-E", provider: "entsoe.eu", url: "https://transparency.entsoe.eu", envKey: "ENTSOE_API_KEY", plan: "free", rateLimit: "400 req/min", expiry: null, category: "Energy", status: has("ENTSOE_API_KEY") ? "active" : "no_key", notes: "European electricity grid — 35 countries, day-ahead prices, generation mix" },
    { name: "Kandilli Observatory", provider: "koeri.boun.edu.tr", url: "https://api.orhanaydogdu.com.tr/deprem", envKey: "-", plan: "open", rateLimit: "Generous", expiry: null, category: "Disasters", status: "active", notes: "Turkish earthquake data from Boğaziçi University Kandilli Observatory" },
    { name: "ESPN Sports", provider: "espn.com", url: "https://site.api.espn.com", envKey: "-", plan: "open", rateLimit: "Generous", expiry: null, category: "Sports", status: "active", notes: "Live scores, news — Premier League, NBA, NFL, MLB, NHL, F1" },
    { name: "Supply Chain DB", provider: "github.com/tstromberg", url: "https://github.com/tstromberg/supplychain-attack-data", envKey: "-", plan: "open", rateLimit: "Unlimited", expiry: null, category: "Cyber", status: "active", notes: "Known software supply chain attacks catalog" },
    { name: "Iran Market", provider: "accessban.com", url: "https://api.accessban.com", envKey: "-", plan: "open", rateLimit: "Generous", expiry: null, category: "Finance", status: "active", notes: "Dollar-Toman, gold, currency rates in Iranian market" },
    { name: "EIA", provider: "eia.gov", url: "https://api.eia.gov/v2", envKey: "EIA_API_KEY", plan: "free", rateLimit: "Generous", expiry: null, category: "Energy", status: has("EIA_API_KEY") ? "active" : "no_key", notes: "US petroleum, natural gas, coal, electricity data" },
    { name: "CryptoPanic", provider: "cryptopanic.com", url: "https://cryptopanic.com/api", envKey: "CRYPTOPANIC_API_KEY", plan: "free", rateLimit: "Generous", expiry: null, category: "Crypto News", status: has("CRYPTOPANIC_API_KEY") ? "active" : "no_key", notes: "Crypto news aggregator with sentiment" },
    { name: "CryptoConvert", provider: "coinconvert.net", url: "https://api.coinconvert.net", envKey: "-", plan: "open", rateLimit: "Generous", expiry: null, category: "Crypto", status: "active", notes: "Real-time crypto-to-fiat conversion" },
    { name: "Tardis.dev", provider: "tardis.dev", url: "https://api.tardis.dev", envKey: "TARDIS_API_KEY", plan: "freemium", rateLimit: "Free tier", expiry: null, category: "Crypto", status: has("TARDIS_API_KEY") ? "active" : "no_key", notes: "Crypto market data — liquidations, order books, trades" },
    { name: "Electricity Maps", provider: "electricitymaps.com", url: "https://app.electricitymaps.com", envKey: "ELECTRICITYMAPS_API_KEY", plan: "free", rateLimit: "Free tier", expiry: null, category: "Energy", status: has("ELECTRICITYMAPS_API_KEY") ? "active" : "no_key", notes: "Global electricity CO2 intensity — 80+ countries, real-time carbon data" },
    { name: "Ransomware.live", provider: "ransomware.live", url: "https://api.ransomware.live", envKey: "-", plan: "open", rateLimit: "Generous", expiry: null, category: "Cyber", status: "active", notes: "Ransomware group tracking — recent victims, groups, attack trends" },
    { name: "Bellingcat SAR", provider: "bellingcat.com", url: "https://github.com/bellingcat/sar-interference-tracker", envKey: "-", plan: "open", rateLimit: "Unlimited", expiry: null, category: "OSINT", status: "active", notes: "SAR satellite interference detection — live GPS jamming zones from Sentinel-1" },
    { name: "Football Data", provider: "football-data.org", url: "https://api.football-data.org", envKey: "FOOTBALL_DATA_API_KEY", plan: "free", rateLimit: "10 req/min", expiry: null, category: "Sports", status: has("FOOTBALL_DATA_API_KEY") ? "active" : "no_key", notes: "Football league standings, match results — Premier League, La Liga, Bundesliga, Serie A, CL" },
    { name: "TV Screener", provider: "tradingview.com", url: "https://scanner.tradingview.com", envKey: "-", plan: "open", rateLimit: "Generous", expiry: null, category: "Finance", status: "active", notes: "Market screener — top gainers, losers, most active stocks and crypto" },
    { name: "Market Indices", provider: "yahoo.com", url: "https://query1.finance.yahoo.com", envKey: "-", plan: "open", rateLimit: "~2,000 req/hour", expiry: null, category: "Finance", status: "active", notes: "Major indices (S&P 500, NASDAQ, DAX, FTSE, Nikkei, BIST 100) batch quotes" },
    // ── Session 4: New open sources ──
    { name: "SpaceX", provider: "spacexdata.com", url: "https://api.spacexdata.com", envKey: "-", plan: "open", rateLimit: "Unlimited", expiry: null, category: "Tech", status: "active", notes: "SpaceX upcoming launches, missions, rockets" },
    { name: "Launch Library 2", provider: "thespacedevs.com", url: "https://ll.thespacedevs.com", envKey: "-", plan: "open", rateLimit: "15 req/hr", expiry: null, category: "Tech", status: "active", notes: "All space launches worldwide — NASA, ESA, SpaceX, ISRO, etc." },
    { name: "NVD CVE", provider: "nvd.nist.gov", url: "https://services.nvd.nist.gov", envKey: "-", plan: "open", rateLimit: "5 req/30s", expiry: null, category: "Cyber", status: "active", notes: "NIST vulnerability database — high-severity CVEs, CVSS scores" },
    { name: "Formula 1 (Ergast)", provider: "jolpi.ca", url: "https://api.jolpi.ca/ergast", envKey: "-", plan: "open", rateLimit: "Generous", expiry: null, category: "Sports", status: "active", notes: "F1 race results, driver standings, season schedule" },
    { name: "Good News Network", provider: "goodnewsnetwork.org", url: "https://www.goodnewsnetwork.org", envKey: "-", plan: "open", rateLimit: "RSS", expiry: null, category: "Good News", status: "active", notes: "Positive news — good news, innovations, sustainability" },
  ];
}
